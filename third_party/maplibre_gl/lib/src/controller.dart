...
class MapLibreMapController extends ChangeNotifier {
  MapLibreMapController({
    ...
    _maplibrePlatform.onMapStyleLoadedPlatform.add((_) async {
      ...
      final previous = _styleInitialization;
      final completion = Completer<void>();
      _styleInitialization = completion.future;
      bool isCurrentStyle() => !isDisposed && generation == _styleGeneration;

      try {
        // Style callbacks can overlap while a source is being added. Finish
        // the old attempt before disposing/replacing its annotation managers.
        await previous;
        ...
        if (isCurrentStyle()) onStyleLoadedCallback?.call();
      } on PlatformException catch (error) {
        // A reload can invalidate the native style after a callback was sent.
        // Its next loaded callback will initialize fresh managers.
        if (error.code != 'styleNotFound' && !isDisposed) rethrow;
      } finally {
        completion.complete();
      }
    });
  ...
  Future<void> _styleInitialization = Future<void>.value();
