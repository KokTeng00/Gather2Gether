    ...
    UIGestureRecognizerDelegate
{
    ...
    private var currentStyleString: String?
    ...
    init(
        ...
        currentStyleString = (args as? [String: Any])?["styleString"] as? String
        ...
        // Inline JSON may finish synchronously inside MLNMapView.init, before
        // its delegate is attached. Adopt that loaded style instead of forcing
        // a second load just to receive the callback.
        if !isMapReady, let style = mapView.style {
            self.mapView(mapView, didFinishLoading: style)
        }
    }
    ...
    func setCameraTargetBounds(bounds: MLNCoordinateBounds?) {
        // The default native map wraps horizontally. Assigning world bounds
        // switches it to Screen constraint mode and stops panning at the date
        // line. Leave that default alone for an unbounded map.
    ...
    func setStyleString(styleString: String) {
        // Creation sends the style both at the top level and in options.
        // Loading it a second time clears the style while Dart is adding its
        // annotation sources, causing an unhandled styleNotFound exception.
        guard styleString != currentStyleString else { return }
